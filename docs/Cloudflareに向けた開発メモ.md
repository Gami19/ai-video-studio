...
ブラウザ内の **ネイティブデコーダ（HTMLVideoElement + Canvas）** でフレーム抽出し、AI API には数枚の画像だけ送ることで API コストを最小化（Phase 1 は **ffmpeg.wasm 非依存**）。
...
│  Hono + TypeScript                                          │
│  /api/analyze  /api/generate  /api/edit-plan                 │
├─────────────────────────────────────────────────────────────┤
│  Web標準API (ブラウザ内・サーバーコスト0)                      │
├─────────────────────────────────────────────────────────────┤
│  Gemini 2.5 Flash   │  Imagen 4 / Replicate                 │
│  (映像理解・激安)    │  (サムネイル生成)                      │
└─────────────────────────────────────────────────────────────┘
```
...
| **Web標準API** | フレーム抽出・（将来の編集は）ブラウザで完結。サーバー処理なし |
| **Gemini 2.5 Flash** | テキスト分析・画像理解（@google/genai） |
| **Imagen 4** | $0.04/枚。Replicate も選択肢 |
...
[1] 動画アップロード (ブラウザ内)
        ↓
[2] HTMLVideoElement + Canvas でフレーム抽出
    └── 均等に 3〜5枚だけ抽出 (コスト削減の肝)
        ↓
[3] POST /api/analyze (Workers)
    └── Gemini 2.5 Flash に画像送信
...
### フェーズ2: 動画編集（Geminiで編集プラン生成）

- Gemini（Workers）で **編集プラン（JSON）** を生成 → ブラウザで **WebCodecs + Canvas** によりレンダリング
- 動画と動画をつなげる（クロスフェード等）→ **WebCodecs + Canvas** でブラウザ内完結（無音MP4）
- 画像素材から演出付きの動画を作る（ズーム/パン/フェード等）→ **WebCodecs + Canvas** でブラウザ内完結（無音MP4）
- **サーバーコスト: 0円**
...
│   │   ├── components/
│   │   │   ├── VideoUploader.tsx
│   │   │   ├── (FrameExtractor は lib に集約) # Phase 1 は ffmpeg.wasm は使わない
│   │   │   └── ThumbnailResult.tsx
│   │   ├── lib/
│   │   │   ├── api.ts              # backend API 呼び出し
│   │   │   ├── frameExtractor.ts   # HTMLVideoElement + Canvas でフレーム抽出
│   │   │   └── ...
│   │   ├── App.tsx
│   │   └── main.tsx
│   ├── index.html
...
export async function extractFrames(
  ...
  // 実装は `frontend/src/lib/frameExtractor.ts` を参照:
  // HTMLVideoElement にシーク → Canvas に描画 → JPEG base64 を生成
  throw new Error("この資料では実コードを省略します");
}
```
...
app.post('/api/generate', async (c) => {
  ...
  // base64 をそのまま返す、または R2 等にアップロードして URL を返す
  return c.json({ imageBase64: imageBytes });
});
```

### （参考）クロスフェードの考え方（ffmpeg ではなく WebCodecs 方針）

```typescript
// 方針: VideoDecoder でデコードしたフレームを Canvas 上で alpha ブレンドし、
// VideoEncoder で再エンコードして MP4（当面無音）として出力する。
```
...
| 機能 | 推奨 | 理由 |
|------|------|------|
| **クロスフェード（トランジション）** | ✅ やる | コスト0、WebCodecs + Canvas で完結（当面無音MP4） |
| **画像素材から演出付き動画** | ✅ やる | コスト0、WebCodecs + Canvas で完結（当面無音MP4） |
...
**方針**: まず Web 標準 API ベースで機能を作り、後で AI オプションを追加する段階的アプローチ。
...
1. **環境変数**: Wrangler の `[vars]` または `wrangler secret put`（GEMINI_API_KEY のみ）
...
Week 1:
  ...
  [ ] HTMLVideoElement + Canvas でフレーム抽出UI（frontend）
  [ ] Gemini 2.5 Flash 接続テスト（backend /api/analyze）

Week 2:
  [ ] Imagen 4 接続（backend /api/generate）
  [ ] CORS・環境変数設定
